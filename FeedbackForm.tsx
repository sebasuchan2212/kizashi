...
interface InputFormProps {
  onSubmit: (input: KizashiInput) => void | Promise<void>;
  disabled?: boolean;
}
...
export function InputForm({ onSubmit, disabled = false }: InputFormProps) {
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    setError('');
    await onSubmit({ domain, question, stateSummary, timeHorizon, emotionIntensity });
  };
    ...
    <section className="panel form-panel" aria-labelledby="form-title">
      ...
      <form onSubmit={submit} className="kizashi-form">
        ...
        <label>
          ...
          <input disabled={disabled} value={question} onChange={(event) => setQuestion(event.target.value)} maxLength={120} />
        </label>

        <label>
          <span>現状の事実と気持ち</span>
          <textarea disabled={disabled} value={stateSummary} onChange={(event) => setStateSummary(event.target.value)} rows={6} maxLength={500} />
        ...
        <div className="form-row">
          <label>
            <span>見たい時間軸</span>
            <select disabled={disabled} value={timeHorizon} onChange={(event) => setTimeHorizon(event.target.value as TimeHorizon)}>
        ...
        <button className="primary-button" type="submit" disabled={disabled}>{disabled ? '診断中...' : 'KIZASHIを読む'}</button>
      </form>
    </section>
  );
}
